...
import (
	...
	v1alpha2info "github.com/quanxiang-cloud/implant/pkg/client/apis/core/v1alpha2"
...
// List lists all Servings in the indexer.
func (s *servingLister) List(selector labels.Selector) (ret []*v1alpha2.Serving, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Serving))
	})
	return ret, err
}
...
// List lists all Servings in the indexer for a given namespace.
func (s servingNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Serving, err error) {
...
// Get retrieves the Serving from the indexer for a given namespace and name.
func (s servingNamespaceLister) Get(name string) (*v1alpha2.Serving, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2info.Resource("serving"), name)
	}
	return obj.(*v1alpha2.Serving), nil
}
